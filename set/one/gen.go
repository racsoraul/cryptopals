...
// This program generates the distribution of English letters. It can be invoked by running `go generate`.
package main
...
func main() {
	...
	outputBuilder.WriteString("// Code generated by go generate; DO NOT EDIT.\npackage one\n\n")
	outputBuilder.WriteString("var EnglishLettersDistribution = map[byte]float64{\n")
	for _, frequency := range distribution {
		outputBuilder.WriteString(fmt.Sprintf("    '%c': %g,\n", frequency.letter, frequency.value))
	}
	outputBuilder.WriteString("}\n")

	f, err := os.Create("englishdistribution.go")
	if err != nil {
		log.Fatal(err)
	}
	defer f.Close()
	f.WriteString(outputBuilder.String())
	if err != nil {
		log.Fatal(err)
	}
}
